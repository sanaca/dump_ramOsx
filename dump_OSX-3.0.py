...
def fct_dump_RAM():
	RAM = input("Choisissez un nom pour le dump de la RAM :")
	volumes = os.popen("ls /Volumes").read()
	print (volumes)
	locate = input("Dans quel volume voulez-vous le copier?")
...
def fct_artefact():
	...
	print (volumes)
	locate = input("Dans quel volume voulez-vous le copier?")
	fichier = input("Choisissez un nom pour le fichier tar.gz de sortie :")
	with open(RAM,"wb") as outfile:
		shutil.copyfile('osxpmem', '/tmp/osxpmem')
		shutil.copytree('pmem.kext', '/tmp/pmem.kext')
		output = subprocess.Popen(["cd /tmp && chmod +x pmem.kext osxpmem && ./osxpmem --format raw /Volumes/'%s'/'%s'" % (locate, RAM) ], shell=True)
...
def fct_checksum():
	...
	rootDir = '/'
	fichier = input("Entrez le nom du json en sortie : ")
...
while var_attack != "q":
	...
	if var_version == "10" :
		...
		print_red("Votre version d'OSX est: Ventura / 13.7")
	elif var_version == "23" :
		print_red("Votre version d'OSX est: Sonoma / 14.8")
	elif var_version == "24" :
		print_red("Votre version d'OSX est: Sequoia / 15.7")
	elif var_version == "25" :
		print_red("Votre version d'OSX est: Tahoe / 26.4")
	else:
